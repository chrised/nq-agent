...
then
    if [ ! -n "$(command -v systemctl)" ] || [[ ! $(systemctl) ]]
    then
        ...
        # Systemd is active
        echo "No crontab found, but this is a systemd machine - timers can be used instead."
    fi
fi

# Check if cron is running and we aren't systemd
if [ -z "$(ps -Al | grep cron | grep -v grep)" ] && [ ! -n "$(command -v systemctl)" ] || [[ ! $(systemctl) ]]
then
...
then
    ...
    then
        ...
        systemctl stop nq-agent.timer
        systemctl disable nq-agent.timer
    fi
fi
...
# Download agent
echo -e "|   Downloading nq-agent.sh to /etc/nodequery\n|\n|   + $(wget -nv -o /dev/stdout -O /etc/nodequery/nq-agent.sh --no-check-certificate https://raw.github.com/$GITHUB_PATH/master/nq-agent.sh)"
